...
def build_libtcc1_archive(
    ...
    if host[0] == "Windows":
        ...
        if host[1] == "ARM64":
            # Storage for upstream's AArch64-only `extern`
            # declarations of `__argc` / `__argv` / `__wargv` /
            # `_environ` / `_wenviron`. Generated by setup.py
            # alongside the win32/lib extract.
            sources.append(TINYCC_DIR / "win32" / "lib" / "c5_win_arm64_args.c")
    if host[1] == "aarch64":
