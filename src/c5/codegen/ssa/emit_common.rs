...
fn push_reloc_term<'a>(term: &'a str, neg: bool, out: &mut alloc::vec::Vec<(bool, &'a str)>) -> bool {
    let t = term.trim();
    if t.is_empty() {
        return false;
    }
    if let Some(inner) = enclosed_by_parens(t) {
        if has_top_level_addsub(inner) {
...
fn parse_reloc_expr(a: &str) -> Result<AsmSectionValue, alloc::string::String> {
    ...
    if names == 2 && dots == 0 && addend.is_empty() {
        if let (Some(m), Some(s)) = (base, neg_name) {
