...
pub(crate) enum AsmSectionItem {
    ...
    /// explicit fill byte; `None` selects the default (the target NOP in an
...
impl AsmExprValue {
    ...
    fn combine(mut self, rhs: Self, sub: bool) -> Result<Self, alloc::string::String> {
        ...
        // distance; fold them so the bases free up.
        if let (Some(p), Some(n)) = (&self.pos, &self.neg)
            && let (Some((ps, po)), Some((ns, no))) = (&p.space, &n.space)
            && ps == ns
        {
            self.add += po - no;
            self.pos = None;
...
/// Parse the operands of `.balign` / `.p2align` / `.align`:
/// `spec[, fill[, max]]`. GNU as allows an empty fill field
...
pub(crate) fn parse_align_operands(rest: &str) -> Option<(i64, Option<u8>, Option<u32>)> {
    ...
    let fill = field(fields.next())?.map(|v| v as u8);
...
fn parse_section_item(
    ...
    match tok {
        ...
        ".p2align" => {
            let (e, fill, max) = parse_align_operands(rest)
                .filter(|&(e, _, _)| (0..=12).contains(&e))
                .ok_or_else(|| alloc::format!("inline asm: bad alignment `{rest}`"))?;
            Ok(AsmSectionItem::Align {
                n: 1 << e,
                fill,
...
/// measurement map and the expression spaces use it.
...
fn val_relational(
    ...
    loop {
        ...
        *i += len;
        ...
        v = AsmExprValue::abs(if rel(val_abs(v, "comparison")?, val_abs(rhs, "comparison")?) {
            -1
        } else {
            0
        });
...
fn val_unary(
    ...
    match b.get(*i) {
        ...
        // `%N` / `%cN` / `%PN`: an operand's compile-time constant.
        Some(b'%') => {
            *i += 1;
