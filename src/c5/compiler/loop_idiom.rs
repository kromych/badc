...
//! header over `dst[i] = value;` (fill) become
...
//! for-init declares it, C99 6.8.5.3).
//!
...
//! * one effect only: the body is a single assignment of the shape
//!   above, so the loop calls nothing, stores nowhere else, and has no
//!   exit of its own;
...
//! * non-overlap for the copy (C99 7.21.2.1): both bases name declared
//!   array objects and the names differ. A copy through pointers is
//!   left alone -- an ascending element loop is not `memmove` when the
//!   destination is above an overlapping source, so the move form is
//!   no fallback. TODO: prove disjointness from `restrict` (C99
//!   6.7.3.1) once the type encoding records it, and version the loop
//!   on a runtime disjointness test for the rest;
...
use super::types::{
    UNSIGNED_BIT, VOLATILE_MASK, is_bool_ty, is_floating_ty, is_struct_value_ty, is_unsigned_ty,
    narrow_const_int,
};
...
impl Compiler {
    ...
    fn const_operand(&self, e: ExprId) -> Option<i64> {
        match self.ast.expr(e) {
            ...
            Expr::Cast { child, to_ty } => {
                ...
                if is_floating_ty(*to_ty) || is_struct_value_ty(*to_ty) {
                    return None;
                }
                let bytes = self.size_of_type(*to_ty);
                i64::try_from(narrow_const_int(
                    bytes,
                    is_unsigned_ty(*to_ty),
                    is_bool_ty(*to_ty),
                    i128::from(v),
                ))
                .ok()
    ...
    fn match_body(&self, id: StmtId, loop_: &Counted) -> Option<Transfer> {
        ...
        if let Some((src, _, src_ty)) = self.match_subscript(rhs, loop_) {
            ...
            // Distinct declared array objects cannot overlap; nothing
            // else here proves it.
            match (dst_object, src_object) {
                (Some(a), Some(b)) if a != b => {}
                _ => return None,
            }
            return Some(Transfer {
                op: MemTransferOp::Copy,
                dst: dst_base,
                dst_object,
                src: src_base,
                fill_byte: None,
    ...
    fn build_transfer(&mut self, loop_: &Counted, xfer: &Transfer, pos: SrcPos) -> Option<ExprId> {
        ...
        let bytes = match (loop_.start, loop_.limit_const) {
            // A count that does not fit stays a loop, as does one the
            // loop's own test proves empty.
            (Some(s), Some(l)) if l > s => Some(l.checked_sub(s)?.checked_mul(xfer.width)?),
            ...
            && mem_transfer_fits(xfer.op, n, align)
        {
            ...
            let src = match xfer.op {
                MemTransferOp::Copy => offset(self, xfer.src),
                _ => self.fill_operand(xfer, pos),
            };
    ...
    /// Replace the loop with `{ init; if (cond) { transfer; i = limit; } }`.
    fn commit(&mut self, id: StmtId, loop_: &Counted, call: ExprId, pos: SrcPos) {
        let mut items: Vec<BlockItem> = Vec::new();
        let xfer = self.ast.push_stmt(Stmt::Expr(call), pos);
        items.push(BlockItem::Stmt(xfer));
        if !loop_.declares_iv {
            ...
            let s = self.ast.push_stmt(Stmt::Expr(assign), pos);
            items.push(BlockItem::Stmt(s));
        }
        let then_s = self.ast.push_stmt(Stmt::Compound(items), pos);
        ...
        outer.push(BlockItem::Stmt(guarded));
