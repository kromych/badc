...
impl Compiler {
    ...
    // The eight initializer-handling methods (collect_array_initializer,
    // pack_initializer_into_data, parse_constant_init_value,
    // collect_struct_initializer, write_init_value,
    // write_array_init_into_data, emit_local_array_init,
    // emit_local_init_store) live in `compiler/initializer.rs` --
    // see the module doc-comment there for the call graph.

    // Local-decl helpers (parse_function_body_local_decl,
    // allocate_static_local, allocate_local_with_init,
    // local_storage_slots) live in `compiler/locals.rs`.
