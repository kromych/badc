        ...
        while self.lex.tk != 0 {
            ...
            while self.lex.tk != ';' && self.lex.tk != '}' {
                ...
                if is_typedef {
                    ...
                    // GNU `__attribute__((aligned(N)))` on the typedef sets the
                    // aliased type's alignment (peek, not take, so a comma list
                    // shares it); the per-declaration reset clears it after.
