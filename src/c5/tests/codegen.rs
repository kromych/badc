...
/// Emit a two-function program and confirm both functions appear as
/// sized `STT_FUNC` symbols.
#[test]
fn defined_functions_get_sized_symtab_entries() {
    use crate::{NativeOptions, Target};
    let program =
        super::compile_str("int helper(int x){return x*x;} int main(){return helper(7);}");
    let bytes = crate::c5::codegen::emit_native_single_tu_for_test(
        &program,
        Target::LinuxX64,
        NativeOptions::default(),
