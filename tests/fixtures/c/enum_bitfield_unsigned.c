...
// the field (a real-world shape). The PE targets take MSVC's rule: an
// enum is `int`, so there its bit-field reads signed and a value whose top
// bit is set within the field sign-extends.
...
int main(void) {
    struct ViaTypedef t;
    t.k = K6;
    if ((int)t.k != 6 - TOP_BIT) return 1; // 6 must not read as -2
    t.k = K4;
    if ((int)t.k != 4 - TOP_BIT) return 2;
    t.k = K2;
    if ((int)t.k != 2) return 3;

    struct Direct d;
    d.k = D5;
    if ((int)d.k != 5 - TOP_BIT) return 4; // 5 must not read as -3
