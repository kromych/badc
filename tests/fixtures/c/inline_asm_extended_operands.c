...
static unsigned long shl_double(unsigned long l, unsigned long r, int c) {
    // shld count, src, dst: dst <<= count, feeding in the high bits of src.
    __asm__("shld %b2, %1, %0" : "+r"(l) : "r"(r), "ci"(c));
    return l;
}
static unsigned long shr_double(unsigned long l, unsigned long r, int c) {
...
static unsigned long bswap64(unsigned long v) {
    __asm__("bswapq %0" : "=r"(v) : "0"(v)); // size-modifier register name
    return v;
}
static unsigned long tsc_read(void) {
    unsigned long t;
...
int main(void) {
    ...
    if (bswap64(0x0102030405060708UL) != 0x0807060504030201UL) {
        return 4;
    }
