// A byte-access leaf taking a pointer parameter, called in a tight
// loop -- the shape crypto compression functions are written in. The
// byte-merge pass collapses each helper body to a wide access, and the
// inliner then has to see that collapsed body: measuring the callee on
// its instruction arena instead of on what the emit issues left the
// superseded per-byte tree in the count and kept every helper out of
// line, so the hot loop paid a call per byte group.
//
// The asm snapshots carry the lock: `mix` reads and writes its state
// with wide accesses and no call.
//
