...
// `volatile` keeps the source operand a runtime value so the
// truncate-and-sign-extend sequence is emitted rather than folded.
static unsigned rtu(unsigned v) { volatile unsigned t = v; return t; }

int main() {
    // (signed char) of unsigned char value with high bit set.
    unsigned char ub = (unsigned char)rtu(0xFF);
    int r1 = (signed char)ub;
    if (r1 != -1) return 1;

    unsigned char ub2 = (unsigned char)rtu(0x80);
    int r2 = (signed char)ub2;
    if (r2 != -128) return 2;

    unsigned char ub3 = (unsigned char)rtu(0x7F);
    int r3 = (signed char)ub3;
    if (r3 != 127) return 3;

    // (signed char) of an unsigned int that fits in 8 bits.
    unsigned int ui = rtu(0xFFu);
    int r4 = (signed char)ui;
    if (r4 != -1) return 4;

    // (signed char) of an unsigned int that overflows 8 bits:
    // truncates to low 8 bits then sign-extends.
    unsigned int ui2 = rtu(0x12345678u);
    int r5 = (signed char)ui2;
    if (r5 != 0x78) return 5;

    unsigned int ui3 = rtu(0x1234ABFFu);
    int r6 = (signed char)ui3;
    if (r6 != -1) return 6;

    // (short) of unsigned short with high bit set.
    unsigned short us = (unsigned short)rtu(0xFFFFu);
    int r7 = (short)us;
    if (r7 != -1) return 7;

    unsigned short us2 = (unsigned short)rtu(0x8000u);
    int r8 = (short)us2;
    if (r8 != -32768) return 8;

    // (short) of unsigned int wider than 16 bits.
    unsigned int ui4 = rtu(0x12345678u);
    int r9 = (short)ui4;
    if (r9 != 0x5678) return 9;

    unsigned int ui5 = rtu(0x1234FFFFu);
    int r10 = (short)ui5;
    if (r10 != -1) return 10;
    ...
    // round-trips.
    signed char sc = (signed char)rtu((unsigned)-42);
    ...
    // unsigned char[].
    unsigned char arr[3];
    arr[0] = (unsigned char)rtu(0xFF);
    arr[1] = (unsigned char)rtu(0x42);
    arr[2] = (unsigned char)rtu(0x10);
