...
// range. Returns 0 when every result matches.
...
__attribute__((noinline)) static unsigned long wrap(unsigned long n) {
    if (n) return wrap(n - 1) - 7;
    return 5;
}

__attribute__((noinline)) static unsigned long times8(unsigned long n) {
...
int main(void) {
    ...
    if (wrap(10) != 5ul - 70ul) return 4;
    if (times8(21) != 0x8000000000000000ul) return 5;
