...
// 16-byte TCB rounded up to it, and on macOS the block is a malloc
// allocation. With the alignment recorded as 8 the 16-byte boundary held
// only while the image size was a multiple of 16; here the objects total
// 40 bytes. Returns 0, distinct non-zero per failure.
