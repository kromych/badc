...
// value context. QEMU's throttle.c relies on this via
// `ARRAY_SIZE(to_check[THROTTLE_READ])` over a `to_check[THROTTLE_MAX][4]`
// table (util/throttle.c). The same holds for a `*p` pointer-to-array row
// deref and a string literal. Returns 0 on success; distinct non-zero per
// fail.
