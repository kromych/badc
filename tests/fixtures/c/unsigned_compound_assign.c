...
// `volatile` keeps the operands runtime values so the compound-assign
// lowering is emitted rather than folded.
static unsigned rtu(unsigned v) { volatile unsigned t = v; return t; }
static unsigned long rtul(unsigned long v) { volatile unsigned long t = v; return t; }

int main() {
    // unsigned int += int constant. Must add the constant
    // verbatim, not constant * sizeof(int).
    unsigned int a = rtu(100);
    a += 5;
    ...
    unsigned char c = (unsigned char)rtu(200);
