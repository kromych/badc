
libc_fp_return_value.x64:	file format elf64-x86-64

Disassembly of section .text:

<.text>:
               	movq	(%rsp), %rdi
               	leaq	0x8(%rsp), %rsi
               	callq	0x4004f6 <.text+0x136>
               	movq	%rax, %rdi
               	callq	*0xfd49(%rip)           # 0x410120
               	pushq	%rbp
               	movq	%rsp, %rbp
               	subq	$0x50, %rsp
               	movq	%rbx, (%rsp)
               	movq	%r12, 0x8(%rsp)
               	movq	%r14, 0x10(%rsp)
               	movslq	%edi, %rbx
               	leaq	0xfd36(%rip), %r9       # 0x410130
               	movq	%rbx, %r8
               	shlq	$0x3, %r8
               	addq	%r8, %r9
               	movq	(%r9), %r8
               	cmpq	$0x0, %r8
               	je	0x400445 <.text+0x85>
               	leaq	0xfd15(%rip), %r9       # 0x410130
               	movq	%rbx, %r8
               	shlq	$0x3, %r8
               	addq	%r8, %r9
               	movq	(%r9), %r8
               	movq	%r8, %rcx
               	movq	(%rsp), %rbx
               	movq	0x8(%rsp), %r12
               	movq	0x10(%rsp), %r14
               	movq	%rcx, %rax
               	addq	$0x50, %rsp
               	popq	%rbp
               	retq
               	leaq	-0x18(%rbp), %r9
               	xorq	%r12, %r12
               	leaq	0xfcf5(%rip), %rdi      # 0x410148
               	movq	%rdi, (%r9)
               	leaq	-0x18(%rbp), %rsi
               	addq	$0x8, %rsi
               	leaq	0xfce6(%rip), %rdi      # 0x41014e
               	movq	%rdi, (%rsi)
               	leaq	-0x18(%rbp), %r9
               	addq	$0x10, %r9
               	leaq	0xfcd8(%rip), %rdi      # 0x410155
               	movq	%rdi, (%r9)
               	leaq	-0x18(%rbp), %rsi
               	movq	%rbx, %rdi
               	shlq	$0x3, %rdi
               	addq	%rdi, %rsi
               	movq	(%rsi), %r14
               	movq	%r12, %rdi
               	movq	%r14, %rsi
               	xorl	%eax, %eax
               	callq	0x4008a7 <dlsym>
               	cmpq	$0x0, %rax
               	je	0x4004c7 <.text+0x107>
               	leaq	0xfc7e(%rip), %r14      # 0x410130
               	movq	%rbx, %r12
               	shlq	$0x3, %r12
               	addq	%r12, %r14
               	movq	(%rax), %r12
               	movq	%r12, (%r14)
               	jmp	0x4004c7 <.text+0x107>
               	leaq	0xfc62(%rip), %r12      # 0x410130
               	shlq	$0x3, %rbx
               	addq	%rbx, %r12
               	movq	(%r12), %rbx
               	movq	%rbx, %rcx
               	movq	(%rsp), %rbx
               	movq	0x8(%rsp), %r12
               	movq	0x10(%rsp), %r14
               	movq	%rcx, %rax
               	addq	$0x50, %rsp
               	popq	%rbp
               	retq
               	pushq	%rbp
               	movq	%rsp, %rbp
               	subq	$0x70, %rsp
               	movq	%rbx, (%rsp)
               	movq	%r12, 0x8(%rsp)
               	movl	$0x1, %r11d
               	movl	%r11d, -0x8(%rbp)
               	movabsq	$0x4010000000000000, %rbx # imm = 0x4010000000000000
               	movq	%rbx, %xmm0
               	xorl	%eax, %eax
               	callq	0x4008ad <sqrt>
               	movq	%xmm0, %rax
               	movabsq	$0x4000000000000000, %rbx # imm = 0x4000000000000000
               	movq	%rax, %xmm14
               	movq	%rbx, %xmm15
               	ucomisd	%xmm15, %xmm14
               	setne	%al
               	movzbq	%al, %rax
               	setp	%r11b
               	movzbq	%r11b, %r11
               	orq	%r11, %rax
               	cmpq	$0x0, %rax
               	je	0x400573 <.text+0x1b3>
               	xorq	%rbx, %rbx
               	movl	%ebx, -0x8(%rbp)
               	jmp	0x400573 <.text+0x1b3>
               	movabsq	$0x400599999999999a, %r12 # imm = 0x400599999999999A
               	movq	%r12, %xmm0
               	xorl	%eax, %eax
               	callq	0x4008b3 <floor>
               	movq	%xmm0, %rax
               	movabsq	$0x4000000000000000, %r12 # imm = 0x4000000000000000
               	movq	%rax, %xmm14
               	movq	%r12, %xmm15
               	ucomisd	%xmm15, %xmm14
               	setne	%al
               	movzbq	%al, %rax
               	setp	%r11b
               	movzbq	%r11b, %r11
               	orq	%r11, %rax
               	cmpq	$0x0, %rax
               	je	0x4005d3 <.text+0x213>
               	xorq	%r12, %r12
               	movl	%r12d, -0x8(%rbp)
               	jmp	0x4005d3 <.text+0x213>
               	movabsq	$0x4002666666666666, %rbx # imm = 0x4002666666666666
               	movq	%rbx, %xmm0
               	xorl	%eax, %eax
               	callq	0x4008b9 <ceil>
               	movq	%xmm0, %rax
               	movabsq	$0x4008000000000000, %rbx # imm = 0x4008000000000000
               	movq	%rax, %xmm14
               	movq	%rbx, %xmm15
               	ucomisd	%xmm15, %xmm14
               	setne	%al
               	movzbq	%al, %rax
               	setp	%r11b
               	movzbq	%r11b, %r11
               	orq	%r11, %rax
               	cmpq	$0x0, %rax
               	je	0x400632 <.text+0x272>
               	xorq	%rbx, %rbx
               	movl	%ebx, -0x8(%rbp)
               	jmp	0x400632 <.text+0x272>
               	movabsq	$0x400c000000000000, %r12 # imm = 0x400C000000000000
               	movq	%r12, %xmm14
               	movabsq	$-0x8000000000000000, %r11 # imm = 0x8000000000000000
               	movq	%r11, %xmm15
               	xorpd	%xmm15, %xmm14
               	movsd	%xmm14, 0x18(%rsp)
               	movsd	0x18(%rsp), %xmm0
               	xorl	%eax, %eax
               	callq	0x4008bf <fabs>
               	movq	%xmm0, %rax
               	movq	%rax, %xmm14
               	movq	%r12, %xmm15
               	ucomisd	%xmm15, %xmm14
               	setne	%al
               	movzbq	%al, %rax
               	setp	%r11b
               	movzbq	%r11b, %r11
               	orq	%r11, %rax
               	cmpq	$0x0, %rax
               	je	0x4006af <.text+0x2ef>
               	xorq	%r12, %r12
               	movl	%r12d, -0x8(%rbp)
               	jmp	0x4006af <.text+0x2ef>
               	movabsq	$0x401c000000000000, %rbx # imm = 0x401C000000000000
               	movabsq	$0x4010000000000000, %r12 # imm = 0x4010000000000000
               	movq	%rbx, %xmm0
               	movq	%r12, %xmm1
               	xorl	%eax, %eax
               	callq	0x4008c5 <fmod>
               	movq	%xmm0, %rax
               	movabsq	$0x4008000000000000, %r12 # imm = 0x4008000000000000
               	movq	%rax, %xmm14
               	movq	%r12, %xmm15
               	ucomisd	%xmm15, %xmm14
               	setne	%al
               	movzbq	%al, %rax
               	setp	%r11b
               	movzbq	%r11b, %r11
               	orq	%r11, %rax
               	cmpq	$0x0, %rax
               	je	0x40071e <.text+0x35e>
               	xorq	%r12, %r12
               	movl	%r12d, -0x8(%rbp)
               	jmp	0x40071e <.text+0x35e>
               	movslq	-0x8(%rbp), %r12
               	cmpq	$0x0, %r12
               	je	0x40073d <.text+0x37d>
               	movl	$0xb, %eax
               	movq	%rax, -0x48(%rbp)
               	jmp	0x400749 <.text+0x389>
               	xorq	%rax, %rax
               	movq	%rax, -0x48(%rbp)
               	jmp	0x400749 <.text+0x389>
               	movq	-0x48(%rbp), %rax
               	movq	%rax, %rcx
               	movq	(%rsp), %rbx
               	movq	0x8(%rsp), %r12
               	movq	%rcx, %rax
               	addq	$0x70, %rsp
               	popq	%rbp
               	retq
               	addb	%al, (%rax)
